...
// Persona a buscar
let nombre = prompt("Indique la persona que quiere buscar:");
let registro = personas[nombre];
let registro_ok = "S";
...
if (registro_ok == "N") {
	//body.innerHTML = `<div id="mensajeEOF"><h1 id="mensajeError"></h1></div>`;
	body.innerHTML = `<div id="mensajeEOF"></div>`;
	let div = document.getElementById("mensajeEOF");
	div.innerHTML += `<h1 id="mensajeError"></h1>`;
	let msgError = document.getElementById("mensajeError");
	msgError.innerText = "No se ha encontrado a " + nombre;
} else {
	body.innerHTML = `<div id="fichaPersona" style="text-align: center;"></div>`;
	let div = document.querySelector("#fichaPersona");	
	//Añadimos mas campos de salida
	div.innerHTML += `<img id="personImg">`;
	div.innerHTML += `<h1 id="personName"></h1>`;
	div.innerHTML += `<p id="personAge"></p>`;
	div.innerHTML += `<p id="personEmail"></p>`;
	div.innerHTML += `<p id="personAdress"></p>`;
	//Asignammos valores a campos
	let idPersonImg = document.querySelector("#personImg");	
	idPersonImg.src = registro.picture;
	document.querySelector("#personName").innerText="Nombre: " + registro.name;
	document.querySelector("#personAge").innerText="Edad: " + registro.age;
	document.querySelector("#personEmail").innerText="Email: " + registro.email;
	//referenciando al body - sel anteriormente
	body.querySelector("#personAdress").innerText="Dirección: " + registro.address;
	body.style.backgroundColor = registro.eyeColor;
};
